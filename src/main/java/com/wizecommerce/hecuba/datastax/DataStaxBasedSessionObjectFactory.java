...
public class DataStaxBasedSessionObjectFactory {
	...
	// Initializing egarly since its very light weight, until clusters and session are built. Guarantees thread safety.
	...
	private Cluster getCluster(ClusterCachingKey key) {
		...
		if (cluster == null) {
			...
			if (properties.containsKey("dataCenter")) {
				loadBalancingPolicy = new DCAwareRoundRobinPolicy((String) properties.get("datacenter"));
			} else {
			...
			if (properties.containsKey("port")) {
				builder.withPort((Integer) properties.get("port"));
			}

			if (properties.containsKey("username") && properties.containsKey("password")) {
				builder.withCredentials((String) properties.get("username"),
						(String) properties.get("password"));
			}

			if (properties.containsKey("compressionEnabled")
					&& (Boolean) properties.get("compressionEnabled")) {
			...
			if (properties.containsKey("readTimeout")) {
				...
				socketOptions.setReadTimeoutMillis((Integer) properties.get("readTimeout"));
			}
			...
			if (properties.containsKey("connectTimeout")) {
				...
				socketOptions.setConnectTimeoutMillis((Integer) properties.get("connectTimeout"));
			}
			...
			if (properties.containsKey("maxConnectionsPerHost")) {
				poolingOptions = new PoolingOptions();
				Integer maxConnectionsPerHost = (Integer) properties.get("maxConnectionsPerHost");
